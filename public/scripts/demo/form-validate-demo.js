$( function(){
	...
	$('#basic-validate').validate({
		...
		showErrors: function() {
			...
			if( errors ){
				...
				// remove existing badge error summary
				$panel.find( '.panel-title > .badge' ).remove();
				// Display a summary of invalid fields as a badge
				$panel.find( '.panel-title' ).prepend( '<span class="badge badge-danger pull-right">' + errors + ' field(s) are invalid</span>' );
			} else{
				// change panel state
	...
	$( '#state-validate' ).validate({
		...
		highlight: function(element, errorClass, validClass){
			...
			$formGroup.removeClass( 'has-success' )
				.addClass( 'has-error' );
			$icon.setClass( 'fa fa-times form-control-feedback' );
		},
		unhighlight: function(element, errorClass, validClass){
			var $formGroup = $(element).closest( '.form-group' ),
				$icon = $formGroup.children( '.form-control-feedback' );
			...
			$icon.setClass( 'fa fa-check form-control-feedback' );
		},
		showErrors: function() {
			var $form = $( this.currentForm ),
				errors = this.numberOfInvalids();
			...
			if (step) {
				$( '#indicator-fills .progress-text' ).text( step + '/' + inputs );
			}
		}
	});
	...
	var advanceValidate = $( '#adv-validate' ).validate({
		...
		highlight: function(element, errorClass, validClass){
			...
			if ( $( element ).is('[name="datepicker"]') ) {
				$formGroup.children( '.form-control-feedback' )
					.setClass( 'fa fa-times form-control-feedback' );
			}
		},
		unhighlight: function(element, errorClass, validClass){
			var $formGroup = $(element).closest( '.form-group' );
			...
			if ( $( element ).is('[name="datepicker"]') ) {
				$formGroup.children( '.form-control-feedback' )
					.setClass( 'fa fa-check form-control-feedback' );
			}
		},
		showErrors: function() {
			var $form = $( this.currentForm ),
	...
	// initial elements
	// select2
	$( '#s2Dropdown, #s2Multi' ).select2({
		placeholder: 'Select...'
	});
	$( '#s2Tags' ).select2({
		tags: ['php', 'ios', 'javascript', 'ruby', 'android', 'kindle', 'html5', 'css3', 'java'],
		placeholder: 'Select...'
	});
	$( '#s2Dropdown, #s2Multi, #s2Tags' ).on( 'change', function(){
